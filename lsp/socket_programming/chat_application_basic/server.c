...
int main()
{
	...
	struct sockaddr_un ser_addr, clt_addr;

	// Create a socket
	ser_fd = socket(AF_LOCAL,SOCK_STREAM,0);

	// Naming newly created socket
	ser_addr.sun_family = AF_LOCAL;
	strcpy(ser_addr.sun_path, "my_socket");
	int ser_len = sizeof(ser_addr);

	//assigning a name and fd to socket using bind
	bind(ser_fd, (struct sockaddr *) &ser_addr, ser_len);

	// listening the socket for any connections
	listen(ser_fd,1);

	// If connecction available,accept the connection
